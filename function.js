...
document.addEventListener("DOMContentLoaded", function () {
    function appendSearchIcon() {
        ...
        if (secteurSelect) {
            secteurSelect.parentNode.querySelector('span').appendChild(searchIconSpan.cloneNode(true));
        }

        if (activiteSelect) {
            activiteSelect.parentNode.querySelector('span').appendChild(searchIconSpan.cloneNode(true));
        }
    }
    ...
    [secteurSelect, activiteSelect].forEach(select => {
        if (select) {
            ...
            select.addEventListener("change", function () {
                toggleIconVisibility(select, icon);
            });
        }
    });
});
